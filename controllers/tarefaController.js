// Controlador para lidar com operações relacioandas às tarefas
...
//Função para atualizar uma taarefa existente
const atualizarTarefa = (req, res) => {
  const { id } = req.params; // Ombtém o ID da tarefa a ser atualizada dos parâmetros da URL
...
const excluirTarefa = (req, res) => {
  ...
  if (index !== -1) {
    ...
    res.status(404).json({ mensagem: "Tarefa não encontrada" }); // Retorna um erro 404 se atarefa não foi encontrada
  }
};
